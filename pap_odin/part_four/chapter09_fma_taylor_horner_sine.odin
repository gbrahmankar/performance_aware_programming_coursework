...
@(private="file")
fma_taylor_horners_sine_approximation :: proc(max_power: u64, x: f64) -> f64 {
	result: f64 = 0

    x_squared: f64 = x * x
    for inv_power: u64 = 1; inv_power <= max_power; inv_power += 2 {
        power: u64 = max_power - (inv_power - 1)
        result = intrinsics.fused_mul_add(result, x_squared, taylor_sine_coefficient(power))
    }
    result *= x
...
chapter09_fma_taylor_horner_sine :: proc() {
    using pap_common
    ...
    for power: u64 = 1; power <= 31; power += 2 {
		for math_op_precision_tester_try_setup_next_precision_test(&tester, 0.0, PI64/2, 1000) {
			...
			math_op_precision_tester_test_using_latest_precision_test(&tester,
	            reference_value,
	            fma_taylor_horners_sine_approximation(power, tester.input_value),
	            "fma_taylor_horners_sine_approximation")
	    }
	}
